...
<div class="container py-4">
  ...
  <div class="d-flex gap-2 border-bottom border-secondary pb-2 mb-4">
    <button type="button" class="tab-btn ${activeTab == 'popular' ? 'active' : ''}" onclick="switchTab(this,'popular')">인기 드라마</button>
    <button type="button" class="tab-btn ${activeTab == 'onair' ? 'active' : ''}" onclick="switchTab(this,'onair')">방영 중</button>
  ...
  <div id="tab-onair" style="${activeTab == 'onair' ? '' : 'display:none;'}">
  ...
  <div class="d-flex justify-content-center mt-5 gap-2">
    <c:if test="${page > 1}">
      <a id="prevPageLink" href="${ctp}/movie/tv?page=${page-1}&country=${country}&tab=${activeTab}" class="btn btn-outline-secondary">이전</a>
    </c:if>
    <span class="btn btn-success disabled">${page} 페이지</span>
    <a id="nextPageLink" href="${ctp}/movie/tv?page=${page+1}&country=${country}&tab=${activeTab}" class="btn btn-outline-secondary">다음</a>
  </div>
</div>
...
function updatePaginationLinks(tabName) {
    ...
    if (prevLink) {
        prevLink.href = '${ctp}/movie/tv?page=${page-1}&country=' + country + '&tab=' + tabName;
    }
    if (nextLink) {
        nextLink.href = '${ctp}/movie/tv?page=${page+1}&country=' + country + '&tab=' + tabName;
    }
}
