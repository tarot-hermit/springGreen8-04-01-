  ...
  <div class="mt-3">
    <div class="d-flex justify-content-between align-items-center mb-2">
      ...
      <button class="btn btn-link btn-sm text-secondary p-0"
              onclick="deleteAllSearch()">전체 삭제</button>
    </div>
      ...
      <c:choose>
        <c:when test="${not empty searchList}">
          ...
          <div class="d-flex justify-content-center gap-2 mt-4">
            <c:if test="${page > 1}">
              <a href="${ctp}/movie/search?q=${q}&page=${page-1}"
...
<script>
  ...
  function deleteSearch(searchNo) {
      $.ajax({
          url: '${ctp}/movie/search/delete',
          type: 'POST',
          data: { searchNo: searchNo },
          success: function(res) {
              if (res == 'ok') {
                  $('#history-' + searchNo).remove();
              }
          }
      });
  }
  ...
  function deleteAllSearch() {
      ...
      $.ajax({
          url: '${ctp}/movie/search/deleteAll',
          type: 'POST',
          success: function(res) {
              if (res == 'ok') {
                  $('#historyArea').parent().remove();
              }
          }
      });
  }
