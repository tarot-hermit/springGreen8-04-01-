...
<head>
  ...
  <style>
    ...
    .stat-card { background:#1e293b; border-radius:16px; padding:24px; border:1px solid rgba(255,255,255,0.06); }
    ...
    .noti-dot { display:inline-block; width:8px; height:8px; background:#f87171; border-radius:50%; margin-left:4px; vertical-align:middle; }
  </style>
</head>
<body>
    ...
    <ul class="sidebar-menu">
      ...
      <li>
        <a href="${ctp}/admin/reports">
          ...
          <c:if test="${stats.pendingReportCnt > 0}">
	 ...
	 <%-- 인기 검색어 통계 --%>
      <div style="background:#1e293b;border-radius:16px;padding:24px;
        ...
        <c:choose>
            ...
            <table class="table table-borderless mb-0" style="color:#fff;">
              <thead>
                <tr style="color:#64748b;font-size:13px;">
                  ...
                  <th>검색어</th>
                  <th style="text-align:right;">검색 횟수</th>
                </tr>
              </thead>
              <tbody>
                <c:forEach var="kw" items="${popularKeywords}" varStatus="st">
                <tr style="border-bottom:1px solid rgba(255,255,255,0.04);font-size:14px;">
                  <td style="color:${st.index < 3 ? '#fbbf24' : '#475569'}; font-weight:700;">
                    ${st.index + 1}
                  </td>
                  <td>
                    <a href="${ctp}/movie/search?q=${kw.keyword}"
                       style="color:#60a5fa;text-decoration:none;">
                      ${kw.keyword}
                    </a>
                  </td>
                  <td style="text-align:right;color:#94a3b8;">${kw.searchCnt}회</td>
                </tr>
                </c:forEach>
              </tbody>
            </table>
          ...
          <c:otherwise>
            <p style="color:#475569;font-size:13px;">검색 기록이 없습니다.</p>
          </c:otherwise>
        </c:choose>
      </div>
  <div class="main-area">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
      <div style="background:#1e293b;border-radius:16px;padding:24px;border:1px solid rgba(255,255,255,0.06);">
        ...
        <a href="${ctp}/admin/users"   class="btn btn-outline-success w-100 mb-2"><i class="fa fa-users me-2"></i>회원 관리</a>
        <a href="${ctp}/admin/reviews" class="btn btn-outline-primary w-100 mb-2"><i class="fa fa-star me-2"></i>리뷰 관리</a>
        <a href="${ctp}/admin/reports" class="btn btn-outline-danger w-100"><i class="fa fa-flag me-2"></i>신고 관리</a>
      </div>
    </div>
  </div>
</div>
